...
```text
Input: L และ N (1 <= L <= 10, 1 <= N <= 100,000)
Output: หมายเลขแถบของช่องสุดท้าย
Constraints: ปลูก k แถบแรก = ปลูกแนวทแยงไป k×L เส้น
             จำนวนช่องทั้งหมด = 1 + 2 + ... + (k×L) = m(m+1)/2 เมื่อ m = k×L
```
...
Step 1: อ่าน L และ N
Step 2: เริ่ม k = 0
Step 3: เพิ่ม k ทีละ 1 แล้วคำนวณจำนวนช่องใน k แถบแรก = m(m+1)/2 เมื่อ m = k×L
Step 4: หยุดเมื่อจำนวนช่อง >= N
Step 5: พิมพ์ k
```
...
```text
เหมือนแผนแรก ใช้ while วนเพิ่ม k และใช้สูตร m(m+1)//2 นับช่อง
N สูงสุด 100,000 วนแค่ไม่กี่ร้อยรอบ จึงเร็วและใช้ memory น้อย
```
...
```text
ค่าน้อยที่สุด ปลูกแค่ 1 ช่อง ต้องอยู่แถบแรก
```
...
```text
N เกินแถบแรกไป 1 ช่อง ต้องขึ้นแถบที่ 2
```
...
```text
ค่าสูงสุดตามโจทย์ ดูว่าไม่ช้าและได้คำตอบถูก
```

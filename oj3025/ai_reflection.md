...
| OJ submission ID, if submitted | https://ijudge.it.kmitl.ac.th/submissions/576936/overview |
| OJ status | Pass |
...
```text
บางtestcase ที่ไม่ผ่านเพราะอะไร code ไม่ครอบคุมหรือมีbugตรงไหน
```
...
```text
เห็นว่า if ในกรณีแรกพอค่าตัวแปรเข้าและไม่ถูกจึงข้ามไปและไม่เข้ากับกรณีอื่น จึงค่าออกมาว่างเปล่า
```
...
```text
แก้ไขโดยการเพิ่มการดักค่าที่จะหลุดรอดไปมาให้ครอบคุม
```
...
```text
าโค้ดเดิมที่คิดมานั้น โครงสร้างและแนวคิดมาถูกทางแล้วกว่า 80% แค่ปรับตรรกะช่วงเงื่อนไข ก็สามารถผ่านเทสเคสทั้งหมดได้
```

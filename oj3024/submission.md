...
```text
input รับสองค่าเป็น ทศนิยม
output Surprising หรือ not Surprising
constraints ถ้าคะแนนสูงสุด และคะแนนต่ำสุดห่างกันเกินสองคะแนนเป็นSurprising ถ้าไม่เกิรเป็น not Surprising ชึ่งรับค่ามาเป็นค่ารวมทั้ง3คน และค่าที่มากที่สุด
```
...
```text
Step 1: รับค่า total (คะแนนรวม 3 คน) กับ max_score (คะแนนสูงสุด)
Step 2: เอา total - max_score จะได้คะแนนรวมของอีก 2 คนที่เหลือ
Step 3: แบ่งคะแนนให้คนที่สองเยอะที่สุดเท่าที่เป็นไปได้ เพื่อหาคะแนนคนที่สามที่ต่ำที่สุด
Step 4: เอาคะแนนสูงสุด มาลบ คะแนนต่ำสุด ดูผลต่าง (btw)
Step 5: ถ้าผลต่าง > 2 พิมพ์ "g" Surprisinถ้าไม่ใช่ พิมพ์ "Not surprising"
```
...
```text
ทดสอบกรณีที่คะแนนห่างกันมากเกิน 2
```
...
```text
บอกว่ามันรับค่าเป็นทศนิยมด้วย
```

คุณยังทำอะไรด้วยตนเอง

```text
ทำเองทุกอย่าง แค่เอาไอเดียเพื่อนมาปรับ
```
